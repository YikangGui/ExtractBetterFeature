...
def deleteTraindata_byLabel(train_cnn, val_cnn, lowAccLabel_fp):
    lowAccLabel = MyFunction.readVector(lowAccLabel_fp)
...
def load_data_for_training_cnn(batch_size=16, image_size=299, lowAccLabel_fp=None):
    ...
    train = pd.concat([train_A, train_B, train_C], 0)
    label2cnnidx = {label_code: idx for idx, label_code in enumerate(train[1].unique().tolist())}
    cnnidx2label = {idx: label_code for idx, label_code in enumerate(train[1].unique().tolist())}
    train['id'] = range(0, trainA_num+trainB_num+trainC_num)
    ...
    transformer_da = transforms.Compose([
        ...
        transforms.RandomResizedCrop(image_size, (0.8, 1), interpolation=PIL.Image.LANCZOS),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5, 0.5, 0.5],
                             std=[0.5, 0.5, 0.5]),
    ])
...
def train_cnn(model, criterion_cent, tr_loader, va_loader,\
    ...
    if repick:
        print("load model")
        model.load_state_dict(torch.load('../data/classficaData/B3_SEnet_1/pth/irv2_16_0.650898.pth'))
    ...
    # optimizer = torch.optim.RMSprop(model.parameters(), weight_decay=4e-5, lr=0.015)
    ...
    for i_ep in range(n_ep):
        ...
        with torch.no_grad():
            ...
            if acc > best_acc:
                best_acc = acc
                torch.save(model.cpu().state_dict(), '../data/classficaData/B3_SEnet_1/pth/irv2_%d_%f.pth' %(i_ep, best_acc))
                model.to(device)
